...
- (void)initMenuItems {
	...
	menuEntity = [[MenuControl alloc] initWithImageNamed:@"buy.png" location:Vector2fMake(240, 70) centerOfImage:YES type:kControlType_Continue];
	[menuEntities addObject:menuEntity];
	[menuEntity release];
		...
		case kSceneState_Running:
			...
			for (MenuControl *control in menuEntities) {
				if([control state] == kControl_Selected) {
					...
					NSString *url = [NSString stringWithString:@"http://itunes.apple.com/app/secure-appbox-pro-ewallet/id421085459?mt=8"];
