			...
			if (bossLife > 0) {
				...
				if(clearDelay > CLEAR_DELAY)
				{
					if (stage == 0) {
						...
						if (clearDelay == CLEAR_DELAY + 1) {
							sceneAlpha -= _sceneFadeSpeed * 0.1;
							[_sharedDirector setGlobalAlpha:sceneAlpha];
							if(sceneAlpha <= 0.0f) {
								clearDelay = CLEAR_DELAY + 2;
							}
						} else if (clearDelay == CLEAR_DELAY + 2) {
							_sharedGameData.score += 1000;
							_sharedGameData.gameSetting = NO;
							[_sharedGameData setStage:stage + 1];
