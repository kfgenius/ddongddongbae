...
{
	vmap[x][y] = expense;
	if(move_max >= 0 && expense >= move_max)	//이동력 한계일 때는 더 이상 검사하지 않음
	{
		return;
	}
...
{
	//가장 좋은 길을 찾기
	int best_dir = dir_none, best_expense = distancemap[point.x][point.y];
	...
	else if(best_dir == dir_right)
	{
		point.x++;
	}
	else if(best_dir == dir_up)
	{
		point.y--;
	}
	else if(best_dir == dir_down)
	{
		point.y++;
	}
