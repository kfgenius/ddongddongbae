...
void CGame::CreateUnit()
{
	...
	{
		char src_file[80];
		char surface_name[80];
		sprintf(src_file, "User\\Unit%d.png", file_no);
		sprintf(surface_name, "Unit%d", file_no);
		...
		if(fp = fopen(src_file, "rb"))
		{
			...
			jdd->LoadPicture(surface_name, src_file, &pi, true);
...
void CGame::Draw()
{
	...
	for(int i=0; i<unit_max; ++i)
	{
		...
		//출력
		char surface_name[80];
		sprintf(surface_name, "Unit%d", unit[p].GetID());
		ani.GetAni(p)->Draw(backbuffer, surface_name, unit[p].GetRealX() - scroll_x, unit[p].GetRealY() - scroll_y - UNIT_PY);
	}
