...
void CDlg::MakeDlgBox(char* dlgbox)
{
	...
	if(jdd->GetPictureInfo(dlgbox, &pi))
	{
		jdd->DeleteSurface(dlgbox);	//ÀÌ¹Ì ±×¸²ÀÌ Á¸ÀçÇÏ¸é Áö¿ì±â
	}
...
void CDlg::Move(int mov_x, int mov_y)
{
	if(!setting || !setting2)
	{
		printf("Warning : ´ëÈ­Ã¢ÀÌ ÃÊ±âÈ­ µÇ¾î ÀÖÁö ¾Ê½À´Ï´Ù.");
		return;
	}
...
void CDlg::SetFont(JFont *font)
{
	if(font==NULL)
	{
		printf("Warning : ÆùÆ®°¡ Á¸ÀçÇÏÁö ¾Ê½À´Ï´Ù.");
		return;
	}
...
void CDlg::SetOpacity(float opacity)
{
	...
	if(dlgbox == NULL)
	{
		printf("Warning : ´ëÈ­Ã¢ÀÌ ÁöÁ¤µÇ¾î ÀÖÁö ¾Ê½À´Ï´Ù.");
		return;
	}
