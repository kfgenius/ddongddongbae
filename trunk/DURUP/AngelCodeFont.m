...
- (id)initWithFontImageNamed:(NSString*)fontImage controlFile:(NSString*)controlFile scale:(CGSize)fontScale filter:(GLenum)filter {
	self = [self init];
	if (self != nil) {
...
- (void)parseFont:(NSString*)controlFile {
	...
	NSString *contents = [NSString stringWithContentsOfFile:[[NSBundle mainBundle] pathForResource:controlFile ofType:@"fnt"] encoding: NSUTF8StringEncoding
													  error: NULL];
...
- (void)drawStringAt:(CGPoint)point text:(NSString*)text {
	...
	for(int i=0; i<[text length]; i++) {
		...
		point.x += kerningAmount * scale.width;
		...
		if(point.x > 0 - ([charsArray[charID] width] * scale.width) || point.x < [[UIScreen mainScreen] bounds].size.width || point.y > 0 - ([charsArray[charID] height] * scale.height) || point.y < [[UIScreen mainScreen] bounds].size.height) {
			...
			CGPoint newPoint = CGPointMake(point.x, 
										   point.y - ([charsArray[charID] yOffset] + [charsArray[charID] height]) * charsArray[charID].scale.height);
		...
		point.x += [charsArray[charID] xAdvance] * scale.width;
...
- (int)getWidthForString:(NSString*)string {
	...
	for(int index=0; index<[string length]; index++) {
		...
		stringWidth += [charsArray[charID] xAdvance] * scale.width;
	}	
...
- (int)getHeightForString:(NSString*)string {
	...
	for(int i=0; i<[string length]; i++) {
		...
		stringHeight = MAX(([charsArray[charID] height] * scale.height) + ([charsArray[charID] yOffset] * scale.height), stringHeight);
	}	
