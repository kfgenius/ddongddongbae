...
- (id)initWithImage:(NSString*)aImage scale:(CGSize)aScale {
	self = [super init];
	if (self != nil) {
		filter = GL_NEAREST;
		scale = aScale;
...
- (id)initWithImage:(NSString*)aImage scale:(CGSize)aScale filter:(GLenum)aFilter {
	self = [super init];
	if (self != nil) {
		filter = aFilter;
		scale = aScale;
...
- (NSString *)description {
	return [NSString stringWithFormat:@"texture:%d width:%d height:%d texWidth:%d texHeight:%d maxTexWidth:%f maxTexHeight:%f angle:%f scale:%f/%f", [texture name], imageWidth, imageHeight, textureWidth, textureHeight, maxTexWidth, maxTexHeight, rotation, scale.width, scale.height];
}


- (Image*)getSubImageAtPoint:(CGPoint)aPoint subImageWidth:(GLuint)aImageWidth subImageHeight:(GLuint)aImageHeight scale:(CGSize)aScale {
...
- (Image*)copyImageAtScale:(CGSize)aScale {
...
- (void)calculateVerticesAtPoint:(CGPoint)aPoint subImageWidth:(GLuint)aSubImageWidth subImageHeight:(GLuint)aSubImageHeight centerOfImage:(BOOL)aCenter {
	...
	double quadWidth = aSubImageWidth * scale.width;
	double quadHeight = aSubImageHeight * scale.height;
...
- (void)renderAt:(CGPoint)point texCoords:(Quad2f*)tc quadVertices:(Quad2f*)qv {
	...
	if (blendMode == kBlendMode_normal) {
		glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
	} else if (blendMode == kBlendMode_lighten) {
