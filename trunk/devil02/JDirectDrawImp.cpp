...
bool JDirectDrawImp::LoadPictureByID(int idx,char* filename,JPictureInfo* picinfo,bool sysmem,int* pid)
{
	bool ret=LoadPicture(idx,filename,picinfo,sysmem);
	if(ret && pid) *pid=idx;
	return ret;
	...
	while(joblist)
	{
		...
		case JobItem::DeleteSurface:
			if(table[joblist->surf1])
			{
				...
				table[joblist->surf1]->GetSurface()->Release();	//두번 릴리즈해야 완전히 메모리에서 제거
...
/*해쉬값 얻기*/
int JDirectDrawImp::GetHash(char* sz)
{
	return hash(sz);
}