...
bool JDirectDrawImp::DuplicateSurface(char* dest, char* src, int* pid)
{
	Render(true,false);
	int idx=hash(dest);
	if(table[idx]) return false;
	if(!table[hash(src)])return false;	//소스가 없을 때

	LPDIRECTDRAWSURFACE surf;
	lasterr=lpDD->DuplicateSurface(table[hash(src)]->GetSurface(), &surf);

	if(surf==NULL) return false;

	JPictureInfo picinfo;
	GetPictureInfo(src, &picinfo);
	table[idx]=new JPictureInfo(picinfo);
	table[idx]->SetSurface(surf);
