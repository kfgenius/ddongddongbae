...
bool JDirectSoundImp::LoadSound(char* name,char* filename,JSoundInfo* sndinfo)
{
	...
	{
#		define DDC2(x) if(FAILED(lasterr=x)) { CloseHandle(hfile); delete table[key]; free(WaveFmt); return false; }
		...
		if(fileid!=0x46464952) { CloseHandle(hfile); return false; }
		...
		if(dataid!=0x45564157) { CloseHandle(hfile); return false; }
		...
		if((chunkinfo.id&(0x00FFFFFF))!=0x00746D66) { CloseHandle(hfile); return false; }
...
#		define DDC2(x) if(FAILED(lasterr=x)) { delete table[key]; return false; }
		table[key]=new SoundTable;
...
bool JDirectSoundImp::SetSoundInfo(char* name,JSoundInfo* buff)
{
	...
	{
	case DirectSound:
		table[key]->buff->SetVolume((LONG)((buff->GetVolume()-1.0f)*10000));
		table[key]->buff->SetPan((LONG)(buff->GetBalance()*10000));
		break;
	case DirectShow:
		table[key]->pba->put_Volume((long)((buff->GetVolume()-1.0f)*10000));
